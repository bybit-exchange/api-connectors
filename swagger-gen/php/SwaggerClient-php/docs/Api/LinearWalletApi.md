# Swagger\Client\LinearWalletApi
...
$apiInstance = new Swagger\Client\Api\LinearWalletApi(
...
try {
    $result = $apiInstance->linearWalletGetRiskLimit();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling LinearWalletApi->linearWalletGetRiskLimit: ', $e->getMessage(), PHP_EOL;
}
?>
```
