...
class FundingApi(
   ...
   * @return Any
   */
  def fundingMyLastFee(symbol: String): Option[Any] = {
    val await = Try(Await.result(fundingMyLastFeeAsync(symbol), Duration.Inf))
    await match {
   ...
   * @return Future(Any)
   */
  def fundingMyLastFeeAsync(symbol: String): Future[Any] = {
      helper.fundingMyLastFee(symbol)
  }
   ...
   * @return Any
   */
  def fundingPrevRate(symbol: String): Option[Any] = {
    val await = Try(Await.result(fundingPrevRateAsync(symbol), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
   ...
   * @return Future(Any)
   */
  def fundingPrevRateAsync(symbol: String): Future[Any] = {
      helper.fundingPrevRate(symbol)
  }
...
class FundingApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def fundingMyLastFee(symbol: String)(implicit reader: ClientResponseReader[Any]): Future[Any] = {
    ...
    if (symbol == null) throw new Exception("Missing required parameter 'symbol' when calling FundingApi->fundingMyLastFee")
  ...
  def fundingPrevRate(symbol: String)(implicit reader: ClientResponseReader[Any]): Future[Any] = {
    // create path and map variables
    ...
    if (symbol == null) throw new Exception("Missing required parameter 'symbol' when calling FundingApi->fundingPrevRate")
